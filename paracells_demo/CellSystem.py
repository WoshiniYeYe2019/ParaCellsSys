...
class CellSystem:
    ...
    def savePool_epoch(self, saving_file: str):
        """
        ...
        :return: None
        """
        np.save(saving_file + '/epoch' + str(self.epoch) + '.npy', self.__pool)
    ...
    def getObjs(self):
        ...
        if self.__objs is not None:
            ...
            SystemError.raiseError("No custom objects are instantiated.")
            return -1
    ...
    def updateSystem(self, method: AbstractMethod = None, fate_decision=True, incr_epoch=False):
        """
    ...
    def setRecordAttrs(self, *attrs):
        ...
        for i in attrs:
            i = str(i)
            if self.__envParamIds.findName(i) == -1:
                SystemError.raiseError(
                    "Set record attribute failed. Attribute " + i + " does not exist in the environment!")
                return
            else:
                self.__recordAttrs.append(i)
    ...
    def record(self):
        ...
        :return: None
        """
        if len(self.__recordAttrs) == 0:
            SystemError.raiseError(
                "No attribute has been assigned to record yet.Please use CellSystem.setRecordAttrs(self, *attrs) first!")
            return
        if len(self.__recordArray) != 0 and len(self.__recordArray[0]) != len(self.__recordAttrs):
            self.__recordArray = []
        newRecording = []
        for attr in self.__recordAttrs:
            newRecording.append(self.getEnvParam(attr))
        self.__recordArray.append(newRecording)
    ...
    def printRecord(self):
        ...
        :return: None
        """
        print(self.__recordArray)

    def genLineChart(self, x_axis: str, show: bool = False, url: str = None):
        """
        ...
        :return: None
        """
        if x_axis not in self.__recordAttrs:
            SystemError.raiseError("Generate line chart failed. Given x_axis_attribute did not recorded!")
            return
        x = self.__recordAttrs.index(x_axis)
        dim = len(self.__recordAttrs) - 1
        x_data = []
        y_data = []
        y_labels = []
        for i in range(len(self.__recordAttrs)):
            if i == x:
                continue
            yd = []
            for rcd in self.__recordArray:
                yd.append(rcd[i])
            y_data.append(yd)
            y_labels.append(self.__recordAttrs[i])
        for rcd in self.__recordArray:
            x_data.append(rcd[x])

        for i in range(dim):
            plt.plot(x_data, y_data[i], alpha=0.5, linewidth=1, label=y_labels[i])
    ...
    def genScatterChart(self, x_axis: str, show: bool = False, url: str = None):
        """
        ...
        for i in range(dim):
            plt.scatter(x_data, y_data[i], alpha=0.5, s=20, label=y_labels[i])
